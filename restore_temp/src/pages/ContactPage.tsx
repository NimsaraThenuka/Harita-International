    ...
    >
      ...
      <div className="relative pt-24 pb-24 overflow-hidden min-h-[52vh] flex items-end">
        <div className="absolute inset-0">
          ...
          <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, rgba(3,12,26,0.92) 0%, rgba(3,12,26,0.6) 45%, #030C1A 100%)" }} />
        ...
        <motion.div className="absolute top-1/3 right-[8%] w-64 h-64 rounded-full pointer-events-none"
          ...
          animate={{ scale: [1, 1.18, 1] }} transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }} />
        <motion.div className="absolute top-[22%] right-[16%] w-20 h-20 border pointer-events-none"
          style={{ borderColor: `${BL}12`, rotate: 25 }}
          animate={{ rotate: [25, 50, 25] }} transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }} />
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 w-full pb-4">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1] }}>
            ...
            <h1 className="font-black text-[#E8F0F8] mt-3 leading-[1.05] tracking-tight"
              ...
              Get in<br /><span className="text-gradient-blue italic">Touch</span>
            </h1>
            <p className="text-lg mt-6 max-w-xl leading-relaxed font-light" style={{ color: "#4A7A9E", fontFamily: "Outfit, sans-serif" }}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
          ...
          <Reveal direction="left" className="lg:col-span-2">
            ...
            <h2 className="font-black text-[#E8F0F8] mb-10"
              style={{ fontSize: "clamp(1.5rem, 3vw, 2.2rem)", fontFamily: "Playfair Display, serif" }}>
              Contact <span className="text-gradient-blue italic">Form</span>
            </h2>
            ...
            {submitted ? (
              <motion.div
                ...
                style={{ border: `1px solid ${BL}25`, background: "#061525" }}
              >
                <motion.div
                  initial={{ scale: 0.5 }}
                  animate={{ scale: 1 }}
                  ...
                  style={{ background: `${B}14`, border: `2px solid ${BL}` }}
                >
                  <IconCheck className="w-8 h-8" style={{ color: BL }} />
                </motion.div>
                <h3 className="text-2xl font-bold text-[#E8F0F8] mb-3">Message Sent</h3>
                <p className="font-light" style={{ color: "#4A7A9E", fontFamily: "Outfit, sans-serif" }}>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  {[
                    ...
                    <div key={f.name}>
                      <label className="block text-[10px] tracking-[0.3em] uppercase mb-2" style={{ color: "#4A7A9E", fontFamily: "Space Mono, monospace" }}>{f.label}</label>
                      <input
                        type={f.type}
                        name={f.name}
                        ...
                        onBlur={e => (e.target.style.borderColor = `${BL}18`)}
                      />
                    </div>
                  ))}
                </div>
                ...
                <div>
                  <label className="block text-[10px] tracking-[0.3em] uppercase mb-2" style={{ color: "#4A7A9E", fontFamily: "Space Mono, monospace" }}>Company / Organisation</label>
                  <input
                    type="text"
                    name="company"
                    value={form.company}
                ...
                <div>
                  <label className="block text-[10px] tracking-[0.3em] uppercase mb-2" style={{ color: "#4A7A9E", fontFamily: "Space Mono, monospace" }}>Topic *</label>
                  <select
                    name="topic"
                    value={form.topic}
                    onChange={handleChange}
                    ...
                    onBlur={e => (e.target.style.borderColor = `${BL}18`)}
                  >
                    <option value="" disabled style={{ background: "#061525" }}>Select a topic...</option>
                    {topics.map(t => <option key={t} value={t} style={{ background: "#061525" }}>{t}</option>)}
                  </select>
                </div>

                <div>
                  <label className="block text-[10px] tracking-[0.3em] uppercase mb-2" style={{ color: "#4A7A9E", fontFamily: "Space Mono, monospace" }}>Message *</label>
                  <textarea
                    name="message"
                    value={form.message}
                    onChange={handleChange}
                    placeholder="Please describe your enquiry in as much detail as you can..."
                    required
                    rows={7}
          ...
          <Reveal direction="right" className="space-y-5">
            ...
            <h2 className="font-black text-[#E8F0F8] mb-7"
              style={{ fontSize: "clamp(1.4rem, 2.5vw, 2rem)", fontFamily: "Playfair Display, serif" }}>
              Reach Us <span className="text-gradient-blue italic">Directly</span>
            </h2>
              ...
              >
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-9 h-9 flex items-center justify-center"
                    ...
                    <item.Icon className="w-4 h-4" style={{ color: BB }} />
                  </div>
                  <span className="text-[10px] tracking-[0.3em] uppercase font-bold" style={{ color: BL, fontFamily: "Space Mono, monospace" }}>{item.label}</span>
                </div>
                {item.href ? (
                  ...
                  <div className="font-semibold text-sm whitespace-pre-line" style={{ color: "#C8D8E8", fontFamily: "Outfit, sans-serif" }}>{item.value}</div>
                )}
                <div className="text-xs mt-1" style={{ color: "#4A7A9E", fontFamily: "Outfit, sans-serif" }}>{item.sub}</div>
              </motion.div>
            ))}

            <div className="p-6" style={{ background: "#061525", border: `1px solid ${BL}18` }}>
              <div className="text-[10px] tracking-[0.3em] uppercase font-bold mb-4" style={{ color: BL, fontFamily: "Space Mono, monospace" }}>Languages</div>
              <div className="flex gap-5">
                {["English", "Japanese"].map(lang => (
                  <span key={lang} className="flex items-center gap-2 text-sm" style={{ color: "#4A7A9E", fontFamily: "Outfit, sans-serif" }}>
                    <IconCheck className="w-3.5 h-3.5 shrink-0" style={{ color: BL }} />
                    {lang}
                  </span>
                ))}
              </div>
